...
public class Assegna_consegnaDAO {
    ...
    public ArrayList<Assegna_Consegna> trovaTutteAssegnazioniByID(int id) {
        ArrayList<Assegna_Consegna> lista = new ArrayList<>();
    ...
    public ArrayList<Assegna_Consegna> trovaTutteAssegnazioniByIDAutista(int id) {
        ArrayList<Assegna_Consegna> lista = new ArrayList<>();
    ...
    public String conferma(int idConsegna, String codiceConferma, int idAutista, String targa) 
    {
        String sql = """
            SELECT b.codice_conferma, b.nbuono
            FROM assegna_consegna ac
            JOIN buoni_consegna b ON ac.buono = b.nbuono
            WHERE ac.id = ?
        """;
        ...
        try (Connection conn = DriverManager.getConnection(database.URL, database.USER, database.PASSWORD);
            ...
            stmt.setInt(1, idConsegna);
    
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    String codiceDb = rs.getString("codice_conferma");
                    int idBuono = rs.getInt("nbuono");
    
                    if (codiceDb.equals(codiceConferma)) {
                        eliminaAssegnazione(idConsegna);
                        ConsegnaDAO consegnaDAO = new ConsegnaDAO();
                        consegnaDAO.inserisciConsegna(idBuono, idAutista, targa);
    
                        return "Consegna confermata ed eliminata correttamente.";
                    } else {
